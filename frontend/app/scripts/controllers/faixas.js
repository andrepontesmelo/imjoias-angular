...
angular.module('app')
  .controller('FaixasCtrl', ['$scope', 'abas', function($scope, abas) {
    ...
    $scope.dados.original.marcacoes.varejo = {
      'A': 1.21,
      'B': 2.21,
      'C': 3.21,
      'D': 4.49,
    ...
    $scope.dados.original.marcacoes.atacado = {
      'A': 5.21,
      'B': 3.21,
      'C': 3.31,
      'D': 4.19,
    ...
    $scope.removerFaixa = function(faixa) {
      var posicao = $scope.dados.alterando.listaFaixas.indexOf(faixa);

      if (posicao !== -1) {
        $scope.dados.alterando.listaFaixas.splice(posicao, 1);
      }
    ...
    $scope.atualizarAlteracoes = function() {
      $scope.alteracoes = [];
      var exclusoesFaixas = _.difference($scope.dados.original.listaFaixas,
        $scope.dados.alterando.listaFaixas);

      exclusoesFaixas.forEach(function(faixa) {
        $scope.alteracoes.push("Exclusão de faixa " + faixa);
      });

      var inclusoesFaixas = _.difference($scope.dados.alterando.listaFaixas,
        $scope.dados.original.listaFaixas);
